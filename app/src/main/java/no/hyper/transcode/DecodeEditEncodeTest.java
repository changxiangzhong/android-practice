...
public class DecodeEditEncodeTest {
    ...
    private static final boolean WORK_AROUND_BUGS = true;  // avoid fatal codec bugs
    ...
    private void __videoEditTest() {
        ...
        if (DEBUG_SAVE_FILE) {
            ...
            String dirName = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES).toString();
            String fileName = "vedit1_" + mWidth + "x" + mHeight + ".mp4";
            sourceChunks.saveToFile(new File(dirName, fileName));
            sourceChunks.saveMp4(new File(dirName, fileName + ".mp4"));
        }
        ...
        while (!outputDone) {
            ...
            while (true) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    output.setMediaFormat(newFormat);
    ...
    private void assertEquals(String s, int i, int outputCount) {
        if(i != outputCount) {
            fail(s);
        }
    }
    ...
    private void assertEquals(String s, long l, long presentationTimeUs) {
        if (l != presentationTimeUs) {
            fail(s);
        }
    }
